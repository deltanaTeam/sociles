  ...
  public function userfollower(Request $request)
{
    ...
    // Get the user ID from the query or default to the authenticated user's ID
    $user_id = $request->query('user_id', auth('sanctum')->user()->id);
    ...
    // Paginate the followers for the given user_id
    $followers = Follower::where('user_id', $user_id)
        ->with(['followerUser:id,name,photo']) // Eager load follower user details
        ->with(['followedUser:id,name,photo']) // Eager load the name of the follow_id
        ->paginate(10);

    // Format the response to include only relevant fields
$response = $followers->map(function ($follower) {
    return [
        'follower_id' => $follower->follow_id,
        'follower_name' => $follower->followedUser->name ?? 'N/A', // Name of follow_id
        'follower_photo' => $follower->followedUser && $follower->followedUser->photo 
    ? config('app.url') . '/public/storage/userimage/' . $follower->followedUser->photo 
    : null,
 // Default to null if no photo exists
    ];
});
    ...
    public function timeline(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $posts = Posts::where(function ($query) use ($user_id) {
                ...
                // ->skip($request->offset)
                ->orderBy('posts.post_id', 'DESC')->paginate();
            $timeline = [];
            foreach ($posts as $post) {
                ...
                $timeline[] = [
                    ...
                    'post_images' =>  $post->mobile_app_image != null ? get_post_images($post->mobile_app_image) : "",
...
'comments' => Comments::with('user')->where('id_of_type', $post->post_id)->get()->map(function ($comment) {
    if (isset($comment->user) && !empty($comment->user->photo)) {
        // Check if photo already contains a full URL
        if (!filter_var($comment->user->photo, FILTER_VALIDATE_URL)) {
            $comment->user->photo = url('public/storage/userimage/optimized/' . $comment->user->photo);
        }
    }
    return $comment;
}),
      ...
      public function create_story(Request $request)
{
    ...
    if (isset($token) && $token != '') {
        ...
        // Use Carbon::now() for the correct timestamp format
        $data['created_at'] = Carbon::now();  // This will store the current date and time in TIMESTAMP format
        $data['updated_at'] = Carbon::now();  // Similarly, update updated_at to the current timestamp
        ...
        if ($request->content_type != 'text') {
            ...
            if (!empty($media_file)):
                ...
                $media_file_data['created_at'] = Carbon::now();  // Use Carbon for timestamp
                $media_file_data['updated_at'] = Carbon::now();  // Use Carbon for updated_at as well
    ...
    if (isset($token) && $token != '') {
        ...
        foreach ($friendships as $friend) {
            ...
            foreach ($followers as $follower) {
                if ($follower->follow_id == $profile_id) {
            ...
            $friendsList[] = [
                ...
                'name' => $user->name,
                'photo' => get_user_images($user->id, "optimized"),
                'cover_photo' => get_cover_photos($user->id, "optimized"),
                'follow' => $followStatus,
            ];
        }
        ...
        $response['pagination'] = [
            ...
            'next_page_url' => $friendships->nextPageUrl(),
            'prev_page_url' => $friendships->previousPageUrl(),
        ];
...
public function friend_request(Request $request)
{
    ...
    if (isset($token) && $token != '') {
        ...
        $response['friendsList'] = $friendsList;
        $response['pagination'] = [
            'current_page' => $friend_requests->currentPage(),
            'per_page' => $friend_requests->perPage(),
            'total' => $friend_requests->total(),
            'last_page' => $friend_requests->lastPage(),
            'next_page_url' => $friend_requests->nextPageUrl(),
            'prev_page_url' => $friend_requests->previousPageUrl(),
        ];
    } else {
        $response['status'] = false;
        $response['message'] = 'unauthorised access';
  ...
  public function get_all_user_stories(Request $request)
{
    $token = $request->bearerToken();
    $response = [];
    ...
    if (isset($token) && $token != '') {
        ...
        $mediaFiles = Media_files::where('user_id', $user_id)->get();
...
public function get_all_stories(Request $request)
{
    $token = $request->bearerToken();
    $response = [];
    $userStories = []; // Initialize an array to store stories grouped by user

    if (isset($token) && $token != '') {
        // Retrieve all media files with their associated user
        $mediaFiles = Media_files::with('user')
            ->whereDate('created_at', Carbon::today())  // Only get records from today
            ->get();

        // Group stories by user_id
        $groupedStories = $mediaFiles->groupBy('user_id');

        // Loop through each group (user) and prepare the stories
        foreach ($groupedStories as $userId => $stories) {
            $userData = [];
            $userData['user_id'] = $userId;
            $user = $stories->first()->user;  // Get user details from the first story

            // Add user information
            $userData["username"] = $user->name ?? 'Unknown';
            $userData["user_photo"] =get_user_images($user->id, "optimized");
            $userData["stories"] = [];

            // Loop through stories for this user and prepare the media data
            foreach ($stories as $media) {
                $mediaData = [];
                $mediaData["id"] = $media->id;
                $mediaData["file_type"] = $media->file_type;

                // Check if file is an image or video and get the appropriate URL
if ($media->file_type == "image") {
    $mediaData["file_url"] = url('public/storage/story/images/' . $media->file_name);
  	$mediaData["created_at"] = $media->created_at;
} else {
  $mediaData["created_at"] = $media->created_at;
    $mediaData["file_url"] = url('public/storage/story/videos/' . $media->file_name);
}
                ...
                $userData["stories"][] = $mediaData;  // Add the media data to the user's stories
            }

            // Add the user data (with stories) to the main response
    ...
    public function profile(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $postCount = Posts::orderBy('post_id', 'desc')->where('user_id', $user->id)->where('posts.publisher', 'post')->count();
            ...
            foreach ($posts as $key1 => $post) {
                ...
                $response['posts'][$key1]['photo'] =  $post->mobile_app_image != null ? get_post_images($post->mobile_app_image) : "";   
...
// Retrieve user data from the database based on the user_id (you can adjust the fields as needed)
                ...
                $hahaCount = 0;
                $angryCount = 0;
    $comments = Comments::with('user')->where('id_of_type', $post->post_id)->get();

    $response['posts'][$key1]['comments'] = [];
    foreach ($comments as $key2 => $comment) {
        $response['posts'][$key1]['comments'][$key2] = [
            'comment_content' => $comment->description,
            //'created_at' => $comment->created_at->diffForHumans(),
            'user' => [
  'id' => $comment->user ? $comment->user->id : null,
'name' => $comment->user ? $comment->user->name : null,
'photo' => $comment->user && get_user_images($comment->user->id, 'optimized') ? get_user_images($comment->user->id, 'optimized') : null,
                ...
                if ($daysDifference < 7) {
                    // Show "time ago" format
                   $response['posts'][$key1]['posted_on']  = $createdDate->diffForHumans();
                } else {
                    // Show the exact date and time
                    $response['posts'][$key1]['posted_on'] = $createdDate->toDayDateTimeString();
                    // Example format: 'Mon, Jan 1, 2024 12:00 AM'
                }

                 $response['posts'][$key1]['posted_on'] = date('M d \a\t H:i A', strtotime($post->posted_on));
    ...
    if (isset($token) && $token != '') {
        ...
        if ($user) {
            ...
            if ($request->hasFile('photo')) {
                ...
                $photoPath = $photo->store('photos', 'public'); // Store in the "public/photos" directory
                $updateData['photo'] = $photoPath;
            }

            // Convert date_of_birth to timestamp if present
            if (isset($updateData['date_of_birth'])) {
                $updateData['date_of_birth'] = strtotime($updateData['date_of_birth']);
            }
            ...
            $done = $user->update($filteredData);

            if ($done) {
                $response['status'] = 200;
                $response['message'] = 'Your data has been updated successfully.';
                $response['data'] = $filteredData; // Return updated data
            } else {
                $response['status'] = 400;
...
public function followers(Request $request)
{
    ...
    // Paginate the followers
    $followers = Follower::where('user_id', $user_id)
        ->with(['followedUser:id,name,photo']) // Ensure the relationship is properly defined
    ...
    $response = $followers->map(function ($follower) {
        return [
            ...
            'name' => $follower->followedUser->name ?? 'N/A',
'photo' => $follower->followedUser && $follower->followedUser->photo 
    ? config('app.url') . '/public/storage/userimage/optimized/' . $follower->followedUser->photo 
    : null,
        ];
    });
    ...
    public function get_comment(Request $request, $postId)
{
    ...
    if (isset($token) && $token != '') {
        ...
        // Paginate comments (10 comments per page, you can adjust this number)
        $comments = Comments::orderBy('comment_id', 'desc')
            ->where('id_of_type', $postId)
            ->where('parent_id', '0')
            ->paginate(10); // Adjust the number to the page size you want
        ...
        foreach ($comments as $comment) {
            ...
            // Paginate replies (10 replies per comment, adjust as needed)
            $replies = Comments::where('parent_id', $comment->comment_id)
                ->paginate(10); // Adjust number of replies per page

            foreach ($replies as $reply) {
                $userReacts = json_decode($reply->user_reacts, true);

                // Initialize counters for replies
                $likeCount = 0;
                $loveCount = 0;
                $sadCount = 0;
                $hahaCount = 0;
                ...
                // Count occurrences of reactions for replies
                foreach ($userReacts as $react) {
                    switch ($react) {
                        case 'like':
                            $likeCount++;
                ...
                // Calculate total reactions for replies
                $totalReacts = $likeCount + $loveCount + $sadCount + $hahaCount + $angryCount;
        ...
        public function filter(Request $request)
{
    ...
    if (isset($token) && $token != '') {
        ...
        $marketplaceQuery = Marketplace::where('status', 1)->orderBy('id', 'desc');

        // Marketplace search logic
        if (!empty($search) || !empty($location)) {
            $marketplaceQuery->where(function ($query) use ($search, $location) {
                if (!empty($search)) {
        ...
        if (!empty($min) || !empty($max)) {
            $marketplaceQuery->where(function ($query) use ($min, $max) {
                if (!empty($min)) {
        ...
        if (!empty($condition)) {
            $marketplaceQuery->where('condition', $condition);
        }

        if (!empty($category)) {
            $marketplaceQuery->where('category', $category);
        }

        if (!empty($brand)) {
            $marketplaceQuery->where('brand', $brand);
        }

        $marketplaceResults = $marketplaceQuery->get();

        // User search logic
        $userResults = [];
        if (!empty($search)) {
            $userResults = User::where('name', 'like', '%' . $search . '%')
        ...
        // Combine results or return separately
        $response = [
            'users' => $userResults,
        ];
    } else {
...
public function isFollowing(Request $request): \Illuminate\Http\JsonResponse
{
    ...
    // Get the authenticated user ID using Sanctum
    ...
    // Validate follow_id as integer
    $validated = $request->validate([
        'follow_id' => 'required|integer',
    ]);

    // Check if the authenticated user is following the specified follow_id
    $exists = DB::table('followers')
        ->where('user_id', $user->id)
        ->where('follow_id', $validated['follow_id'])
        ->exists();

    return response()->json([
        'exists' => $exists,  // false will be returned if not found
    ...
    public function notifications(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            $user_id = auth('sanctum')->user()->id;
            $date = Carbon::today();
            $new_notification = Notification::where('reciver_user_id', $user_id)
                ->orderBy('id', 'DESC')->get();
            $newnoti = [];
...
public function deleteAccount(Request $request)
{
    ...
    try {
        // Optionally log user details or perform any cleanup tasks here
        $user->delete(); // Delete the authenticated user
